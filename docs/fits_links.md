...
- **Registration:** `fits register link-type <LINK_TYPE> <IN_OBJ_TYPE> <OUT_OBJ_TYPE>`  
  So the first object type after `LINK_TYPE` is the **in** endpoint’s prefix, and the second is the **out** endpoint’s prefix.

## Files and ownership

| Location | Role |
|----------|------|
| `.fits/registry.json` | Machine-owned. Declares object prefixes and **link types** (counters + tombstones). **Do not edit by hand.** |
...
Link rows use ids shaped like `{LINK_TYPE}-{n}` (e.g. `implements-3`), with `n` allocated from the per–link-type counter in `.fits/registry.json`, parallel to object prefixes.

Object ids and link type names **must not overlap** so commands like `fits rm` can disambiguate `REQ-3` from `implements-3`.
...
- **Semantic checks** (during `fits validate`): `link_type` must be registered; `out` / `in` must be issued, non-tombstoned object ids whose prefixes match the registry row for that link type; link id must be issued and not tombstoned.
...
Validation loads the links index, builds **graph edges** (`out` → `in`, tagged with `link_type`), and reports problems such as endpoints missing from loaded object bundles.

## Related documentation

- [Registry (objects + link types)](fits_registry.md) — `.fits/registry.json` fields and load behavior.
